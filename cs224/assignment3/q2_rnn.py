...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) < max_length:
            ret.append((sentence+[zero_vector]*(max_length-len(sentence)), labels+[zero_label]*(max_length-len(sentence)), [True if i<len(sentence) else False for i in range(max_length)]))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings_table = tf.get_variable("embeddings", shape=self.pretrained_embeddings.shape, initializer=tf.constant_initializer(self.pretrained_embeddings))
        embeddings = tf.nn.embedding_lookup(embeddings_table, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        with tf.variable_scope("OUT_POR", reuse=tf.AUTO_REUSE):
            U = tf.get_variable("U", shape=[self.config.hidden_size, self.config.n_classes],
                                                initializer=tf.contrib.layers.xavier_initializer())
            b_2 = tf.get_variable("b_2", shape=[self.config.n_classes], initializer=tf.constant_initializer())
        ...
        self.predi = tf.argmax(preds, axis=2)
        return preds
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(
                                                ...
                                                logits=tf.boolean_mask(preds, self.mask_placeholder))) / tf.to_float(tf.shape(preds)[0])
        ### END YOUR CODE
        return loss
    ...
    def predict_on_batch(self, sess, inputs_batch, mask_batch):
        ...
        predictions = sess.run(self.predi, feed_dict=feed)
        return predictions
