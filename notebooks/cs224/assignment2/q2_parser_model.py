...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        w = tf.Variable(xavier_weight_init()([self.config.n_features*self.config.embed_size, self.config.hidden_size]), dtype=tf.float32)
        b1 = tf.Variable(tf.zeros([self.config.hidden_size],dtype=tf.float32))
        h = tf.nn.relu(tf.matmul(x, w)+b1)
        h_drop = tf.nn.dropout(h, self.dropout_placeholder)
        u = tf.Variable(xavier_weight_init()([self.config.hidden_size, self.config.n_classes]), dtype=tf.float32)
